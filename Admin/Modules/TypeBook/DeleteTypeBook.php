		...
		<div class="row">
          ...
          <?php 
              ...
              { 
                $id=$_GET[ 'id']; 
                $data=[ 'MALOAI'=>$id ]; 
                $name = $type_book->db_get_list_type_name_book($id); 
                if ($type_book->h->is_submit('delete_type') && $type_book->db_delete_type($data)) 
                { 
          ...
          ?>
					<div class="container">
                        ...
                        Bạn có thật sự muốn xóa tên loại sách này không ?
										...
										<p class="card-text text-center"> <b style="font-size:30px;"> <?php  echo $name['LOAISACH']; ?></b>
										</p>
									</div>
									<form action="" method="post">
										<div class="card-footer bg-danger pt-2 pb-2 text-center">
											<input type="hidden" name="request_name" value="delete_type">
											<input type="submit" class="btn btn-info" value="Có"> <a href="<?php echo $type_book->h->get_url('book/admin/?m=typebook&a=listtypebook'); ?>" class="btn btn-info">Không</a>
										</div>
									</form>
								</div>
							</div>
